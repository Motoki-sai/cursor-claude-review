...
| キー | 既定 | 説明 |
|------|------|------|
| `claudePrReview.claudeExecutable` | `claude` | Claude Code CLI のパスまたはコマンド名 |
| `claudePrReview.skillPath` | 空 | `SKILL.md` の上書きパス。空なら拡張同梱 → `~/.claude/skills/review-master/SKILL.md` の順で解決 |
| `claudePrReview.allowedTools` | `Bash(gh *),Bash(git *),Read` | headless 時の `--allowedTools`（権限ルール構文） |
| `claudePrReview.maxTurns` | `40` | `--max-turns` |
| `claudePrReview.outputFormat` | `text` | **ライブストリーム無効時**の `--output-format`（`text` / `json`） |
| `claudePrReview.liveStreamOutput` | `true` | `stream-json` で逐次イベントを解析し、進捗と Report を更新 |
...
|-----|---------|-------------|
| `claudePrReview.claudeExecutable` | `claude` | Path or command name for the Claude Code CLI |
| `claudePrReview.skillPath` | empty | Override path to `SKILL.md`. If empty: bundled extension file, then `~/.claude/skills/review-master/SKILL.md` if present |
| `claudePrReview.allowedTools` | `Bash(gh *),Bash(git *),Read` | `--allowedTools` for headless runs (permission rule syntax) |
| `claudePrReview.maxTurns` | `40` | `--max-turns` |
| `claudePrReview.outputFormat` | `text` | When live stream is off: `--output-format` (`text` / `json`) |
| `claudePrReview.liveStreamOutput` | `true` | Use `stream-json` to parse events incrementally and update progress / Report |
